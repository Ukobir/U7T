...
void ssd1306_draw_pixel(ssd1306_t *ssd, uint8_t x, uint8_t y, bool color) {
  if (x >= ssd->width || y >= ssd->height) return; // Verifica se o pixel está dentro dos limites

  uint16_t index = x + (y / 8) * ssd->width; // Calcula o índice correto na memória

  if (color) {
      ssd->ram_buffer[index + 1] |= (1 << (y % 8)); // Acende o pixel
  } else {
      ssd->ram_buffer[index + 1] &= ~(1 << (y % 8)); // Apaga o pixel
  }
}

void ssd1306_conf_init(ssd1306_t *ssd){
  i2c_init(PORTA_I2C, 400 * 1000);
  gpio_set_function(SDA, GPIO_FUNC_I2C);
  gpio_set_function(SCL, GPIO_FUNC_I2C);
  gpio_pull_up(SDA);
  gpio_pull_up(SCL);
  ssd1306_init(ssd, 128, 64, false, ENDERECO_DISPLAY, PORTA_I2C);
  ssd1306_config(ssd);
...
void telaEsquerda(ssd1306_t *ssd)
{
  ssd1306_fill(ssd, false);                         // Limpa o display
  ssd1306_rect(ssd, 3, 3, 122, 58, true, false);      // Desenha um retângulo
  ssd1306_draw_string(ssd, "Mova para", 25, 18);    // Desenha uma string
  ssd1306_draw_string(ssd, "a Esquerda", 25, 35);      // Desenha uma string
  ssd1306_send_data(ssd);                           // Atualiza o display
}

void telaDireita(ssd1306_t *ssd)
{
  ssd1306_fill(ssd, false);                         // Limpa o display
  ssd1306_rect(ssd, 3, 3, 122, 58, true, false);      // Desenha um retângulo
  ssd1306_draw_string(ssd, "Mova para", 25, 18);    // Desenha uma string
  ssd1306_draw_string(ssd, "a Direita", 25, 35);      // Desenha uma string
  ssd1306_send_data(ssd);                           // Atualiza o display
}

void telaCima(ssd1306_t *ssd)
{
  ssd1306_fill(ssd, false);                         // Limpa o display
  ssd1306_rect(ssd, 3, 3, 122, 58, true, false);      // Desenha um retângulo
  ssd1306_draw_string(ssd, "Mova para", 25, 18);    // Desenha uma string
  ssd1306_draw_string(ssd, "Cima", 45, 35);      // Desenha uma string
  ssd1306_send_data(ssd);                           // Atualiza o display
}
void telaBaixo(ssd1306_t *ssd)
{
  ssd1306_fill(ssd, false);                         // Limpa o display
  ssd1306_rect(ssd, 3, 3, 122, 58, true, false);      // Desenha um retângulo
  ssd1306_draw_string(ssd, "Mova para", 25, 18);    // Desenha uma string
  ssd1306_draw_string(ssd, "Baixo", 45, 35);      // Desenha uma string
  ssd1306_send_data(ssd);                           // Atualiza o display
}

void valores(ssd1306_t *ssd, const char *maxX, const char *maxY, const char *minX, const char *minY)
{
  ssd1306_fill(ssd, false);                         // Limpa o display
  ssd1306_rect(ssd, 3, 3, 122, 58, true, false);      // Desenha um retângulo
  ssd1306_draw_string(ssd, maxX, 92, 30);    // Desenha uma string
  ssd1306_draw_string(ssd, minX, 5, 30);    // Desenha uma string
  ssd1306_draw_string(ssd, maxY, 45, 0);    // Desenha uma string
  ssd1306_draw_string(ssd, minY, 50, 54);    // Desenha uma string
  ssd1306_send_data(ssd);                           // Atualiza o display
}